...
namespace Shaski_Bakhmut
{
    public partial class GameForm : Form
    {
        ...
        private void CheckForWinner()
        {
            ...
            foreach (Checker piece in game.BoardPrevent)
            {
                if (piece != null)
                {
        ...
        private bool SetupPlayers()
        {
            using (SetupForm setupForm = new SetupForm())
            {
                if (setupForm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    //SideType player1Color = setupForm.Player1Color;
                    //SideType player2Color = setupForm.Player2Color;

                    game = new Game();

                    game.CreatePlayer(player1Name);
                    game.CreatePlayer(player2Name);
        ...
        private void InitializePlayerLabels()
        {
            if (game.Players[0].Side == SideType.White)
            {
                player1Label = new Label
                {
                    ...
                    Text = $"{game.Players[0].Name} (Белые)",
                ...
                player2Label = new Label
                {
                    ...
                    Size = new Size(200, 23),
                    Font = new Font(Font.FontFamily, 14),
                    Text = $"{game.Players[1].Name} (Черные)",
                    TextAlign = ContentAlignment.MiddleLeft
                };
            }
            else
            {
                player1Label = new Label
                {
                    ...
                    Size = new Size(200, 23),
                    Font = new Font(Font.FontFamily, 14),
                    Text = $"{game.Players[0].Name} (Черные)",
                    TextAlign = ContentAlignment.MiddleLeft
                };
                player2Label = new Label
                {
                    ...
                    Size = new Size(200, 23),
                    Font = new Font(Font.FontFamily, 14),
                    Text = $"{game.Players[1].Name} (Белые)",
                    TextAlign = ContentAlignment.MiddleLeft
                };
            }
        ...
        private void UpdateTurnList()
        {
            ...
            foreach (var turn in game.Moves)
            {
                string turnString = $"{turn.Checker.Color} {turn.Checker.Type} from ({turn.StartPosition[0]}, {turn.StartPosition[1]}) to ({turn.EndPosition[0]}, {turn.EndPosition[1]})";
